...
const Index = () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                MySwal.fire({
                    ...
                    text: data?.errors?.nid?.[0] || data?.message || "حدث خطأ غير متوقع",
                    icon: "error"
                });
            }
        } catch (err) {
    ...
    return (
        <div dir="rtl" className="min-h-screen bg-gradient-to-bl from-background via-muted/30 to-background font-cairo" >
            {/* Header */}
                ...
                {!image ? (
                    <Card className="border-2 border-dashed border-border hover:border-primary/50 transition-colors">
                        <CardContent className="p-0">
                            <div className="flex flex-col items-center justify-center py-16 px-6 cursor-pointer" onClick={() => fileInputRef.current?.click()} onDragOver={(e) => e.preventDefault()} onDrop={handleDrop} >
                            ...
                            <Input ref={cameraInputRef} type="file" accept="image/*" capture="environment" className="hidden" onChange={(e) => { const file = e.target.files?.[0]; if (file) handleFile(file); }} />
                        </CardContent>
                    </Card>
                ) : (
                    <>
                        ...
                        <Card className="overflow-hidden">
                            <CardContent className="p-0">
                                <img src={image} alt="صورة البطاقة" className="w-full max-h-80 object-contain bg-muted/30" />
                            </CardContent>
                        </Card>
                        {/* Action Buttons */}
                        <div className="flex gap-3">
                            <Button className="flex-1" size="lg" onClick={extractData} disabled={loading} >
                                {loading ? (
                                    <>
                    ...
                    <Card className="border-primary/20 shadow-lg animate-in fade-in slide-in-from-bottom-4 duration-500">
                        ...
                        <CardContent className="space-y-4">
                            ...
                            <div className="space-y-2">
                                ...
                                <input type="number" value={mobile} name="mobile" onChange={(e) => setMobile(e.target.value)} placeholder="ادخل رقم المحمول" className="w-full border rounded-md p-2" />
                            </div>
                            <Button className="w-full mt-10" onClick={saveData}>
                                حفظ البيانات
                            </Button>
                        </CardContent>
                    </Card>
